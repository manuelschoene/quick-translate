...
// Sent with the FullDto of the translation a second launch brought back.
...
func NewAdapter(app *application.App, files *system.FileService) *Adapter {
	...
	application.RegisterEvent[*FullDto](eventRestored)
...
// Called by Wails on shutdown. Closes the database connection.
...
func (a *Adapter) Restore() {
	...
	dto, err := a.latestTranslation()
	if err != nil {
		// An empty or disabled history is no reason for an error view; the window is open either way.
		fmt.Printf("Could not bring the last translation back: %v\n", err)
		return
	}

	a.app.Event.Emit(eventRestored, dto)
}

// Steps to the translation that was stored last and returns the whole state, because the stored translation brings its own provider and languages with it.
func (a *Adapter) latestTranslation() (*FullDto, error) {
	a.mutex.Lock()
	defer a.mutex.Unlock()
	...
	if _, _, _, err := a.core.LatestTranslation(); err != nil {
		return nil, err
	}
